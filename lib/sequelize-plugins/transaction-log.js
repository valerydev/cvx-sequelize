  ...
  function hookTemplate(hookFunc) {
    ...
    return _.wrap(hookFunc||function(){}, function(wrappedFunc){
      ...
      if(sequelize.constructor.cls) {
        ...
        let instance = arguments[1]
        let opts     = arguments[2]

        let session = sequelize.constructor.cls.active.session
        let currTx  = sequelize.constructor.cls.active.transaction
        ...
        if( hookFunc && instance.Model.options.transactionLog ) {
          ...
          let attributes = _.values(instance.rawAttributes).filter(attr => !attr.type.toString().startsWith('VIRTUAL'))
          attributes = _.map(attributes, 'fieldName')

          let data = wrappedFunc.call(null, instance, attributes)
          if(!data) return
