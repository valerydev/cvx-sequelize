...
function assembleSyncData(batch){
  ...
  batch = batch.map( (batchItem, order) => {
    ...
    batchItem = _.clone(batchItem);
    batchItem.order      = order + 1;

    //Marcamos la operacion a ejecutar
    if(data[pkName] === undefined) {
      //Si no se define la clave primaria en la data es un create
      batchItem.op = 'C';
    } else if( _.keys(_.omit(data, pkName)).length > 0  ) {
      //Si ademas de la clave primaria hay mas atributos es un update
      batchItem.op = 'U';
    } else {
      //Si solo esta la clave primaria en la data es un delete
      batchItem.op = 'D';
    }
  ...
  var itemWithContract = syncData.batch.find( batchItem => !_.isUndefined(batchItem.instance.contractId));
  if(itemWithContract) {
  ...
  var scopedItem = syncData.batch.find( batchItem => !_.isUndefined(batchItem.instance.classifierId1));

  if( scopedItem ) {
    //A todos los items les asignamos el mismo nivel de acceso
    syncData.classifierId1 = instance.classifierId1 || 0;
    syncData.classifierId2 = instance.classifierId2 || 0;
    syncData.classifierId3 = instance.classifierId3 || 0;
    syncData.branchId      = instance.branchId      || 0;
  }
